		...
		if (opt.containsKey("chkSpace")) {
			...
			if (requiredSpace > 0) {
				...
				if (available < requiredSpace) {
					MyLog.append(MyLog.ERROR,
							tmpDirString + " only has " + available + " bytes < required " + requiredSpace + " bytes");
					return null;
				}
				if (opt.containsKey("verbose")) {
					MyLog.append(tmpDirString + " has " + available + " bytes >= required " + requiredSpace + " bytes");
				}
			}
		}
